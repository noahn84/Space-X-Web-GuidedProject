...
test("Missions list shows data when rendered with new missions data", () => {
  // Arrange: simulate the behavior of rendering htis component to the page, before we've made an api call
  const {queryAllByTestId, getAllByTestId, rerender} = render(<MissionsList missions={[]}/>);
  // sanity check: make sure that ther are no missions yet when the component mounts
