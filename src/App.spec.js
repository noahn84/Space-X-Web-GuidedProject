...
test("successfully renders data from api", async () => {
  ...
  mockFetchMissions.mockResolvedValueOnce(missionsData); // misionsData is the 'res' in the api call in api/fetchMissions, so res.data must equal missionsList.data, that's why we wrap the array in an object and add the data property which nests the original array
  ...
  // Click on Get Data button (two things should happen):
  //    - Fetching message is rendered
  //    - API call is initiated
  userEvent.click(button); // async operation kicks off here

  await findByText(/we are fetching data/i); // waitFor includes the act() call, and you can await findByText()
  ...
  // anything below await will be run as if its a .then(), which won't run until the async operation has been resolved
  await waitFor(() => {
